...
///
enum class MediaType { AUDIO, VIDEO, SENSOR };
...
struct InferenceResult {
  ...
  /// e.x., resultJson = [{"mediatype": "video", "source": "vid_1", "label": "person",
  /// "precision": 90.2, "xmin": 100, "ymin": 120, "xmax": 300, "ymax": 400, "summary": "not in safe
  /// distance"},
  ///                    {"mediatype": "video", "source": "vid_1", "label": "person", "precision":
  ///                    98, "xmin": 400, "ymin": 520, "xmax": 700, "ymax": 600, "summary": "in safe
  ///                    distance"},
  ///                    {"mediatype": "gyroscope", "source": "gyro_1", "summary": "changing lane"}]
  std::string resultJson;

  /// Vision box result structure 
  /// available or not.
  bool visionBoxAvailable = false;

  /// Detail inferrence result 
  /// data with various annotation and 
  /// representaion e.g. box, line, point.
...
///
/// typedef std::function<void(std::string source, time_t timestamp, unsigned char* data, int len,
/// std::vector<struct InferenceResult> result)>onInferenceResultReceived;
...
public:
  ...
  ///
  virtual void setNetworkModelsFilePath(std::string filePath,
                                        std::string modelConfigParams = "") = 0;
  ///
  ...
  ///
  virtual std::vector<InferenceResult> feedMediaData(MediaType mediaType, std::string source,
                                                    u_int64_t timestamp, MediaInfo mediaInfo,
